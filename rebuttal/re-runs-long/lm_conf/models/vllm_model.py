...
def _adaptive_gpu_memory_utilization(target: float = 0.9, margin: float = 0.08, floor: float = 0.3) -> float:
    """Shrink vLLM's gpu_memory_utilization to fit whatever VRAM is actually
    free right now, instead of assuming exclusive access to the whole GPU.
    Other users' jobs sharing this GPU can otherwise make vLLM's startup
    memory check fail outright (it requires target*total <= free)."""
    try:
        free_bytes, total_bytes = torch.cuda.mem_get_info()
    except Exception:
        return target
    free_frac = free_bytes / total_bytes
    return max(floor, min(target, free_frac - margin))
...
class vLLMModel(AbstractModel):
    ...
    def run_generation(self, prompt_collection: PromptCollection) -> list[ModelOutputs]:
        ...
        base_seed = self.cfg.get("seed")
        vllm_model = LLM(model=self.model_name,
                         max_model_len=self.cfg.get("max_model_len", 4096),
                         gpu_memory_utilization=_adaptive_gpu_memory_utilization(),
                         trust_remote_code=True)

        model_outputs_list = []
        for round_idx in range(self.repeat):
            logging.info(f"vLLM [{self.model_name}] Generation Round {round_idx + 1}/{self.repeat}")
            round_seed = None if base_seed is None else int(base_seed) * 10_000 + round_idx
