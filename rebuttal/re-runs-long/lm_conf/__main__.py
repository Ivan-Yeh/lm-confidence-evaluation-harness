...
if __name__ == "__main__":
    ...
    # Rebuttal rerun-long: write under a dedicated tree so this never collides
    # with (or gets picked up by) production /hdd/ivny/results/ runs of the
    # same dataset/task/model. Falls back to a local results/ dir under this
    # rebuttal/re-runs-long copy if /hdd/ivny/re-runs-long isn't writable.
    ...
    try:
        ...
        path = f"/hdd/ivny/re-runs-long/results/{dataset_name}/{task_name}/{cfg.qa_model.name}/{timestamp}"
    except OSError:
