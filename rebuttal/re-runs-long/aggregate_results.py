...
calibration, across the 4-model x 3-signal ELI5 (long-form) rebuttal rerun.

Reads calibration_performance.csv from each
  {common_dir}/in_domain_calibration/eli5/{model}/
(written by calibration/in_domain_calibration.py) and writes two CSVs next to
this script:
  eli5_ece_fd_by_run.csv   - long format, one row per (model, signal, stage)
  eli5_ece_fd_summary.csv  - same rows, just a tidier column order (single
                              run per model here, so no mean/std across seeds)
"""
import os
import pandas as pd
...
# run_model_chain.sh: a dedicated re-runs-long tree, separate from production
# /hdd/ivny/results/.
if os.path.isdir("/hdd/ivny/re-runs-long/results"):
    common_dir = "/hdd/ivny/re-runs-long/results"
else:
    common_dir = os.path.join(SCRIPT_DIR, "results")


def calib_csv_path(model: str) -> str:
    return os.path.join(
        common_dir, "in_domain_calibration", "eli5",
        model, "calibration_performance.csv",
    )
...
def main():
    ...
    summary_path = os.path.join(SCRIPT_DIR, "eli5_ece_fd_summary.csv")
    by_run_df.to_csv(summary_path, index=False)
    print(f"Saved summary ({len(by_run_df)} rows) -> {summary_path}")
    print(by_run_df.to_string(index=False))
