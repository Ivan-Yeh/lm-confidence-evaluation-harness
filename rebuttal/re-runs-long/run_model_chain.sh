#!/usr/bin/env bash
# Rebuttal rerun-long: ELI5 (sentence-transformers/eli5, first 800 questions)
# long-form direct-QA sampling (lc/tp/su) -> in-domain calibration (100%
# lexicon, 30/70 train-test split, no beta-guided variant), for one model on
# one GPU.
#
# Usage: run_model_chain.sh <GPU> <MODEL> <REASONING_EFFORT>
#   REASONING_EFFORT is "low" or "null" (literal string "null" disables it,
#   matching scripts/local_direct_truthful_qa.sh's per-model convention).
#
# Does, sequentially on the same GPU:
#   1. direct_qa_unified_lc  - fresh generation (repeat=20, temp=1), the only
#      expensive stage; also runs grading + majority-cluster building.
#   2. direct_qa_unified_tp  - cache_path=<lc dir>; reuses cached
#      generations/clusters/grading, only recomputes the tp confidence
#      signal. Cheap.
#   3. direct_qa_unified_su  - same, cheap.
#   4. calibration.in_domain_calibration - reads back lc/tp/su via
#      get_latest_leaf_node (safe here: each model has its own directory, no
#      concurrent runs of the *same* model, so no discovery races).
#
...
# dedicated re-runs-long tree so this never collides with production
# /hdd/ivny/results/ or other rebuttal reruns.
if mkdir -p /hdd/ivny/re-runs-long/results 2>/dev/null; then
  COMMON_DIR=/hdd/ivny/re-runs-long/results
else
...
run_sampling_stage() {
  local task="$1" logfile="$2" cache_path="${3:-}"
  local extra_args=()
...
run_sampling_stage_until_valid() {
  ...
  for stage_attempt in $(seq 1 "$MAX_STAGE_RETRIES"); do
    if ! run_sampling_stage "$task" "$logfile" "$cache_path"; then
...
echo "=== [$(date '+%F %T')] model=$MODEL gpu=$GPU: starting ==="
check_disk_space

# --- lc: fresh generation (expensive) ---
lc_log="$LOG_DIR/${SAFE_MODEL}_lc.log"
echo "  [lc] -> $lc_log"
if ! LC_PATH="$(run_sampling_stage_until_valid direct_qa_unified_lc "$lc_log")"; then
  echo "  !!! FAILED: lc sampling model=$MODEL exhausted $MAX_STAGE_RETRIES stage-attempts (see $lc_log)" >&2
  exit 1
fi
echo "  [lc] results_path=$LC_PATH"

# --- tp: cheap, reuses lc's cache_path ---
tp_log="$LOG_DIR/${SAFE_MODEL}_tp.log"
echo "  [tp] -> $tp_log"
if ! TP_PATH="$(run_sampling_stage_until_valid direct_qa_unified_tp "$tp_log" "$LC_PATH")"; then
  echo "  !!! FAILED: tp sampling model=$MODEL exhausted $MAX_STAGE_RETRIES stage-attempts (see $tp_log)" >&2
  exit 1
fi
echo "  [tp] results_path=$TP_PATH"

# --- su: cheap, reuses lc's cache_path ---
su_log="$LOG_DIR/${SAFE_MODEL}_su.log"
echo "  [su] -> $su_log"
if ! SU_PATH="$(run_sampling_stage_until_valid direct_qa_unified_su "$su_log" "$LC_PATH")"; then
  echo "  !!! FAILED: su sampling model=$MODEL exhausted $MAX_STAGE_RETRIES stage-attempts (see $su_log)" >&2
  exit 1
fi
echo "  [su] results_path=$SU_PATH"

# --- calibration (in-domain, 100% lexicon, no beta-guided, 30/70 split) ---
check_disk_space
calib_log="$LOG_DIR/${SAFE_MODEL}_calibration.log"
calib_out_dir="$COMMON_DIR/in_domain_calibration/$DATASET/$MODEL"
...
if [[ "$calibration_ok" -ne 1 ]]; then
  echo "  !!! FAILED: calibration model=$MODEL exhausted $MAX_STAGE_RETRIES stage-attempts (see $calib_log)" >&2
  exit 1
fi

echo "=== [$(date '+%F %T')] model=$MODEL gpu=$GPU: done ==="
exit $overall_status
