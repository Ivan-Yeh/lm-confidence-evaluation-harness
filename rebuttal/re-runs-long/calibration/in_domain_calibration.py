...
# NOTE: this rebuttal/re-runs-long copy intentionally omits the direct
# beta-guided rewrite/eval path present in the original script -- the task
# only calls for the lexicon-guided rewrite + calibration comparison (no
# beta-guided variant needed for the eli5 long-form rerun).
...
if __name__ == "__main__":
    ...
    # Rebuttal rerun-long: read/write under a dedicated tree so this never
    # collides with (or gets auto-discovered ahead of) production
    # /hdd/ivny/results/ or /hdd/ivny/*_in_domain_calibration/ runs. Falls
    # back to a local results/ dir under this rebuttal/re-runs-long copy if
    # /hdd/ivny/re-runs-long isn't writable.
    RERUNS_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    try:
        os.makedirs("/hdd/ivny/re-runs-long/results", exist_ok=True)
    ...
    ling_path = get_latest_leaf_node(f"{common_dir}/{args.dataset}/{prompt_type}_unified_lc/{args.model}/")
    tp_path = get_latest_leaf_node(f"{common_dir}/{args.dataset}/{prompt_type}_unified_tp/{args.model}/")
    su_path = get_latest_leaf_node(f"{common_dir}/{args.dataset}/{prompt_type}_unified_su/{args.model}/")
    ...
    rewrite_cfg = {
        "rewrite_model": {
            ...
            # Long-form eli5 responses can run up to ~512 tokens; give the
            # rewrite model enough budget to reproduce a comparably long,
            # multi-sentence rewrite instead of truncating it (max_model_len
            # is recomputed per-batch by _update_max_model_len below anyway).
            "max_model_len": 8000,
            "temperature": 1.0,
            "max_tokens": 1024,
            "reasoning_effort": "low",
    ...
    # (the direct beta-guided rewrite path is intentionally omitted here: the
    # task only calls for the lexicon-guided rewrite + calibration
    # comparison, not the beta-guided variant.)
    rewrite_targets = []
    for conf_method in ["calibrated_lc", "calibrated_tp", "calibrated_su"]:
