...
if __name__ == "__main__":
    ...
    # Lexicon-size ablation: LEXICON_FRAC is read by calibration.utils at import
    # time (see rebuttal/lex-size/calibration/utils.py); we re-read it here just
    # to name the output directory consistently with the sampled lexicon.
    lexicon_frac = float(os.environ.get("LEXICON_FRAC", "1.0"))
    lexicon_pct = int(round(lexicon_frac * 100))
    print(f"Lexicon ablation: LEXICON_FRAC={lexicon_frac} ({lexicon_pct}%)")
    ...
    save_dir = f"/hdd/ivny/in_domain_calibration_lex_size/{args.dataset}_{lexicon_pct}/{args.model}/"
    ...
    # (the direct beta-guided rewrite path is intentionally omitted here: it
    # doesn't use the lexicon at all, so it's irrelevant to this lexicon-size
    # ablation and only doubles GPU work.)
    rewrite_targets = []
    for conf_method in ["calibrated_lc", "calibrated_tp", "calibrated_su"]:
