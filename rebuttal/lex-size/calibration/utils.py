...
# --- Lexicon-size ablation ------------------------------------------------
# Controlled via the LEXICON_FRAC env var (e.g. "0.25", "0.5", "0.75", "1.0").
# The full lexicon is shuffled once with a fixed seed (LEXICON_SEED) and then
# truncated to the requested fraction, so smaller fractions are always a
# subset of larger ones (nested ablation).
LEXICON_SEED = int(os.environ.get("LEXICON_SEED", "42"))
LEXICON_FRAC = float(os.environ.get("LEXICON_FRAC", "1.0"))

_lexicon_rng = np.random.default_rng(LEXICON_SEED)
_shuffled_order = _lexicon_rng.permutation(len(LEXICON))
LEXICON = LEXICON.iloc[_shuffled_order].reset_index(drop=True)

if LEXICON_FRAC < 1.0:
    _n_keep = max(1, int(round(LEXICON_FRAC * len(LEXICON))))
    LEXICON = LEXICON.iloc[:_n_keep].reset_index(drop=True)

print(
    f"[lexicon ablation] LEXICON_FRAC={LEXICON_FRAC} LEXICON_SEED={LEXICON_SEED} "
    f"-> using {len(LEXICON)} lexicon entries"
)
