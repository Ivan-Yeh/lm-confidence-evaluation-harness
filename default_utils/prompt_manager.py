...
def direct_free_form_qa(cfg: dict, dataset_manager: DatasetsManager) -> PromptCollection:
    ...
    for _, row in dataset_manager.get_dataset().iterrows():
        ...
        if few_shot > 0:
            few_shot_examples: pd.DataFrame = dataset_manager.get_few_shot_dataset().sample(n=few_shot, random_state=cfg.get("seed", 42))
            for _, example in few_shot_examples.iterrows():
                few_shot_text += "Q: " + question_template.render(**example.to_dict())
    ...
    return PromptCollection(questions=formatted_questions, answer_keys=dataset_manager.get_dataset()['answer'].tolist(), system_prompt=cfg.get("system_prompt", ""), context_texts=prompts, continuation_texts=continuation_texts)